    ...
    <nav aria-label="Breadcrumb">
      <ol className="flex items-center space-x-1 text-sm text-muted-foreground">
        ...
        {displaySegments.map((segment, index) => {
          ...
          let href = `${basePath}/${displaySegments.slice(0, index + 1).join("/")}?role=${role}`;

          // ✅ Fix: Redirect "Ideas" breadcrumb to role-specific dashboard
          if (segment === "ideas") {
            if (role === ROLES.PRINCIPAL) {
              href = `/dashboard/principal/ideas?role=${role}`;
            } else if (role === ROLES.SUPER_ADMIN) {
              href = `/dashboard/admin/ideas?role=${role}`;
            }
          }
