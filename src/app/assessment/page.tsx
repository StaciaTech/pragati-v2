...
import { ChevronLeft, Check, BrainCircuit } from 'lucide-react';
import {
  Dialog,
  DialogContent,
  DialogHeader,
    ...
    if (currentQuestion.next_question_logic) {
      ...
      for (const logic of currentQuestion.next_question_logic) {
        if (logic.condition.includes('>=')) {
          if (score >= parseInt(logic.condition.split('>=')[1].trim())) return logic.next_question_id;
    ...
    if (isFinished) {
        return (
            <div className="text-center p-6 flex flex-col items-center">
                <Check className="w-16 h-16 bg-green-100 text-green-600 rounded-full p-3 mb-4" />
                <h3 className="text-2xl font-bold">Assessment Complete!</h3>
                <p className="text-muted-foreground mt-2">Your analysis is ready. Log in or sign up to view your personalized report.</p>
            </div>
        )
    }
  ...
  return (
    <div className="min-h-screen bg-muted flex items-center justify-center p-4">
      <Card className="w-full max-w-3xl">
        <CardHeader>
          <Progress value={isFinished ? 100 : progress} className="mb-4" />
        ...
        <CardFooter className="flex justify-between">
           {isFinished ? (
              <Button onClick={() => router.push('/login')} className="w-full">
                <BrainCircuit className="mr-2 h-4 w-4" /> Generate Report & View Profile
              </Button>
           ) : (
             <>
