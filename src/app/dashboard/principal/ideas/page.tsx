  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Review and monitor all ideas submitted within your institution.</CardDescription>
      </CardHeader>
      <CardContent>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          ...
          <Select value={filterTtc} onValueChange={setFilterTtc}>
            <SelectTrigger><SelectValue placeholder="Filter by TTC..." /></SelectTrigger>
            <SelectContent>
        ...
        <Table>
          ...
          <TableBody>
            {filteredIdeas.map((idea) => {
              ...
              const internalMentor = MOCK_INTERNAL_MENTOR_USERS.find(m => m.id === (idea as any).internalMentorId);
              return (
                <TableRow key={idea.id}>
                  <TableCell className="font-medium">{idea.id}</TableCell>
                  <TableCell>{idea.title}</TableCell>
                  <TableCell>{idea.innovatorName}</TableCell>
                  <TableCell>{ttc?.name || 'N/A'}</TableCell>
                  <TableCell>{internalMentor?.name || 'N/A'}</TableCell>
                  <TableCell>
                    <Badge className={STATUS_COLORS[idea.status]}>{idea.status}</Badge>
                  </TableCell>
                  <TableCell className="text-right">
                    <Button variant="link" size="sm" asChild>
                      <Link href={`/dashboard/ideas/${idea.id}?role=${ROLES.PRINCIPAL}`}>View Report</Link>
                    </Button>
                  </TableCell>
                </TableRow>
              )
             ...
             {filteredIdeas.length === 0 && (
                <TableRow>
                    <TableCell colSpan={7} className="text-center">No ideas found matching your criteria.</TableCell>
                </TableRow>
             )}
          </TableBody>
        </Table>
