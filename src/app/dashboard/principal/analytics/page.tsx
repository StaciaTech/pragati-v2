...
export default function CollegeAnalyticsPage() {
    ...
    const ttcPerformance = ttcs.map(ttc => {
        ...
        return {
            name: ttc.name.replace('Dr. ', '').replace('Mr. ', '').replace('Ms. ', ''),
            ideas: ttcIdeas.length,
            approvalRate: ttcIdeas.length > 0 ? (approved / ttcIdeas.length) * 100 : 0
        };
    });
       ...
       <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Comparison of idea submissions and approval rates across TTCs.</CardDescription>
          </CardHeader>
          <CardContent>
                    ...
                    <BarChart data={ttcPerformance}>
                        ...
                        <Tooltip content={<ChartTooltipContent 
                            ...
                            formatter={(value, name) => {
                                if (name === 'ideas') return [value, 'Ideas Submitted'];
                                if (name === 'approvalRate') return [`${(value as number).toFixed(1)}%`, 'Approval Rate'];
                                return [value, name];
                            }}
                        />} />
                        <Legend />
                        <Bar yAxisId="left" dataKey="ideas" fill="hsl(var(--chart-1))" name="Ideas Submitted" />
                        <Bar yAxisId="right" dataKey="approvalRate" fill="hsl(var(--chart-2))" name="Approval Rate (%)" />
                    </BarChart>
                </ResponsiveContainer>
              </ChartContainer>
          </CardContent>
